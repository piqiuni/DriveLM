...
class ReadOutput(object):
    ...
    def get_combined_image(self, cimage_name, image_paths, question, answer, ground_truth):
        ...
        for image_path in image_paths:
            n_image_path = image_path[3:]
            abs_image_path = os.path.join(self.abs_data_path, "data", n_image_path)
    ...
    def process(self, evaldata_path):
        data1_dict = self.read_json(evaldata_path)

        common_keys = data1_dict.keys()
        for key in tqdm(common_keys):
            for frame in data1_dict[key]["key_frames"].keys():
                frame_data_qa = data1_dict[key]["key_frames"][frame]["QA"]
                QA_pairs = frame_data_qa["perception"] + frame_data_qa["prediction"] + frame_data_qa["planning"] + frame_data_qa["behavior"]
                image_paths = list(data1_dict[key]["key_frames"][frame]["image_paths"].values())          
                cimage_name = key + "_" + frame + ".png"
                question = QA_pairs[0]["Q"]
                answer = ""
                ground_truth = QA_pairs[0]["A"]
...
def main():
    ...
    """
    evaldata_path = "./pi_test/demo_data/demo_test_eval.json"
